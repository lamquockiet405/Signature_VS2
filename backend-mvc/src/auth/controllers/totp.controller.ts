...
export class TotpController {
  ...
  async verifyTotp(
    ...
    try {
      console.log(
        '🔐 TOTP Verify request received for token:',
        verifyTotpDto.token,
      );
      ...
      const result = await this.totpService.verifyTotp(
        token,
        verifyTotpDto.token,
      );
  ...
  async verifyTokenForAction(
    ...
    try {
      console.log(
        '🔐 TOTP Verify Token request received for token:',
        verifyTotpDto.token,
      );
      ...
      const isValid = await this.totpService.verifyTotpForAuth(
        userId,
        verifyTotpDto.token,
      );
