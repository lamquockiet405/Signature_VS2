...
export class TotpService {
  ...
  async setupTotp(authToken: string) {
    try {
      ...
      throw new HttpException('Failed to setup TOTP', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async verifyTotp(authToken: string, token: string) {
    try {
      ...
      if (!secret) {
        throw new HttpException('TOTP not set up. Please setup TOTP first.', HttpStatus.BAD_REQUEST);
      }

      if (user.totp_enabled) {
        throw new HttpException('TOTP is already enabled for this user', HttpStatus.BAD_REQUEST);
      }
      ...
      throw new HttpException('Failed to verify TOTP', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async verifyTotpForAuth(userId: string, token: string): Promise<boolean> {
    try {
      console.log('🔐 Verifying TOTP for authentication, User ID:', userId, 'Token:', token);
      ...
      if (!user.totp_enabled || !user.totp_secret) {
        throw new HttpException('TOTP not enabled for this user', HttpStatus.BAD_REQUEST);
      }
