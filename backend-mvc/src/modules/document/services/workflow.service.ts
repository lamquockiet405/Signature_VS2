...
export class WorkflowService {
  ...
  async processDelegation(
    ...
    try {
      ...
      console.log('Validating users:', workflowDto.delegator_id, workflowDto.delegate_id);
      ...
      console.log('✅ Delegation workflow created, waiting for delegate to sign');
  ...
  async approveWorkflow(
    ...
    const normalizedDelegator = String(workflow.delegator_id).trim().toLowerCase();
    ...
    if (approveDto.totpToken) {
      ...
      const isTotpValid = await this.totpService.verifyTotpForAuth(currentUserId, approveDto.totpToken);
      if (!isTotpValid) {
  ...
  async buildSigningContext(workflowId: string): Promise<ISigningContext> {
    ...
    const companyResult = await this.db.query('SELECT name FROM company LIMIT 1');
    const companyName = companyResult.rows.length > 0 
