...
async function findLastTextPosition(
  ...
  try {
    ...
    console.log(
      `📐 Estimated Y position: ${y} (page height: ${pageHeight}, lines: ${totalLines})`,
    );
...
export async function addMetadataAndPlaceholder(
  ...
  try {
    ...
    if (lastTextPos.found && lastTextPos.lastLine) {
      ...
      console.log(
        `📍 Positioning metadata after last text: "${lastTextPos.lastLine.substring(0, 30)}..."`,
      );
      ...
      console.log(
        `✅ Signature position: x=${x}, y=${y} (dynamic based on content)`,
      );
    } else {
