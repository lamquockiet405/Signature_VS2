...
import { CreateWorkflowDto, WorkflowType } from '../dtos/create-workflow.dto';
...
export class DocumentManagementService {
  ...
  async createSignature(
    ...
    try {
      ...
      if (workflowType === WorkflowType.DELEGATION) {
        ...
        return {
          message: 'Delegation created. Delegate can now sign the document.',
  ...
  async signDocument(id: string, signData: any, currentUserId: string) {
    try {
      ...
      if (signData.totpToken) {
        ...
        if (!isTotpValid) {
          throw new BadRequestException(
            'Invalid TOTP token. Please enter the correct TOTP code.',
          );
        }
        ...
        const isTotpEnabled =
          await this.totpService.isTotpEnabled(currentUserId);
        if (isTotpEnabled) {
          throw new BadRequestException(
        ...
        console.log(
          '⚠️ No TOTP provided - proceeding without TOTP verification (user has TOTP disabled)',
        );
      }
  ...
  async getFileForDownload(id: string) {
    ...
    try {
      ...
      if (documentSignatureResult.rows.length > 0) {
        const signatureData = documentSignatureResult.rows[0]
          .signature_data as { signed_path?: string } | null;
        if (signatureData?.signed_path) {
          const fs = await import('fs');
          if (fs.existsSync(signatureData.signed_path)) {
            console.log(
              '✅ Found signed file (document_signatures):',
              signatureData.signed_path,
            );
            return {
      ...
      if (signedLogResult.rows.length > 0) {
        ...
        if (metadata?.signed_path) {
          ...
          if (fs.existsSync(metadata.signed_path)) {
            console.log(
              '✅ Found signed file (hsm_signedlogs metadata):',
      ...
      if (activityLogResult.rows.length > 0) {
        const details = activityLogResult.rows[0].details as {
        ...
        if (details?.signedFilePath) {
          ...
          if (fs.existsSync(details.signedFilePath)) {
            console.log(
              '✅ Found signed file (activity_logs):',
              details.signedFilePath,
            );
            return {
  ...
  async signDocumentDirect(
    ...
    try {
      ...
      // Validate TOTP token
      const isValidTotp = await this.totpService.verifyTotpForAuth(
        currentUserId,
        signData.totpToken,
      );
      if (!isValidTotp) {
      ...
      try {
        ...
        console.log(
          '✅ Activity logged with signed file path:',
          signResult.signedPath,
        );
      } catch (logError) {
        console.error(
          '⚠️ Failed to log direct signing activity (non-critical):',
          logError,
        );
      }
      ...
      try {
        ...
        console.error(
          '⚠️ Failed to update document status (non-critical):',
          updateError,
        );
      }
